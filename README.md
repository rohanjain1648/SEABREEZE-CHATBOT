...
- **Context-Aware Intelligence**: Uses a **RAG (Retrieval-Augmented Generation)** pipeline to answer technical project queries (BHKs, Pricing, Amenities) with 100% accuracy.
...
- **Intent-Driven Closing**: Identifies high-intent users and dynamically offers site visit invitations and calls with relationship managers.
- **Modern UI/UX**: A sleek, glassmorphic chat interface built with React 19 and Tailwind CSS 4.
- **Voice Integration**: Ready for voice-based interactions to enhance accessibility.
...
### RAG Pipeline
1. **Data Source**: `project_data.json` contains flattened project specifications.
2. **Embeddings**: Uses `all-MiniLM-L6-v2` (SentenceTransformers) for semantic representation.
3. **Vector Store**: Powered by **FAISS** for lightning-fast similarity search.
...
| **RAG Agent** | Injects real-time project context into the state from the FAISS database. |
| **Sales Agent** | The "Brain"—generates the luxury-toned response using Gemini 2.0. |
